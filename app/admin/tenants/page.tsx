...
export default async function AdminTenantsPage() {
  const list = await db<Profile[]>`select * from profiles where role = 'tenant' order by created_at desc limit 200`;

  return (
    <div>
      <PageHeader title="Tenants" description="All tenant accounts." />
      {list.length === 0 ? (
        <EmptyState icon={<Users className="size-8" />} title="No tenants yet" />
      ) : (
        <Card>
