...
export default async function AgentUnitsPage({ params }: { params: { id: string } }) {
  ...
  if (!prop) notFound();
  if (prop.agent_id !== profile.id && profile.role !== "owner" && profile.role !== "admin") {
  ...
  return (
    <div>
      <PageHeader
        ...
        description={`Mark which units are taken in ${prop.title}`}
        actions={
          <Link
      ...
      <Card>
        <CardContent className="p-5">
          <UnitManager propertyId={params.id} mode="mark" floors={floors} />
        </CardContent>
      </Card>
    </div>
  );
